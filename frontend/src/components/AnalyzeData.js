...
const AnalyzeData = () => {
  ...
  const fetchFiles = useCallback(async () => {
    try {
      ...
      if (savedFileId && response.data.length > 0) {
        ...
        const savedFile = response.data.find(
          (file) => file._id === savedFileId || file.fileId === savedFileId
        );

        console.log(
          "Found matching file:",
          savedFile ? savedFile.filename : "None"
        );
        ...
        if (savedFile) {
          ...
          console.log(
            "Using most recent file instead:",
  ...
  const fetchFileData = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/excel/data/${selectedFile._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const renderDataPreview = () => {
    ...
    if (dataError) {
      return (
        <div className="data-error">
          <p>{dataError}</p>
          <button onClick={fetchFileData} className="retry-button">
            Try Again
          </button>
        </div>
      );
    ...
    if (!fileData || !fileData.data || fileData.data.length === 0) {
      return (
        <div className="no-data-message">No data available for this file.</div>
      );
    }
    ...
    return (
      <div className="data-preview-container">
        <div className="data-preview-header">
          <h3>
            Data Preview:{" "}
            <span className="filename-text">{selectedFile.filename}</span>
          </h3>
        ...
        <div className="data-table-container">
          ...
          <div className="pagination-controls">
            <div className="pagination-info">
              Showing rows {startIndex + 1} to {endIndex} of {totalRows} total
              rows
            </div>
  ...
  const renderDataVisualization = () => {
    ...
    const headers = Object.keys(fileData.data[0]).filter(
      (column) => column && column.trim() !== "" && column !== "__empty"
    );
          ...
          <div className="data-warning-critical">
            ...
            <div className="warning-content">
              <p className="warning-text">
                Both selected columns contain text/non-numeric data.
              </p>
              <p className="warning-suggestion">
                Charts require at least one numeric axis to display data
                properly. Please select a column with numeric values for at
                least one axis.
              </p>
            </div>
          </div>
        ...
        if ((isXNumeric && isYNumeric) || (!isXNumeric && !isYNumeric)) {
          return (
            <div className="data-warning">
              ...
              <div className="warning-content">
                <p className="warning-text">
                  Pie/Doughnut charts work best with one categorical column and
                  one numeric column.
                </p>
                <p className="warning-suggestion">
                  For best results, select one column with categories (text) and
                  one with values (numbers).
                </p>
              </div>
            </div>
          );
        ...
        if (!isYNumeric) {
          return (
            <div className="data-warning">
              ...
              <div className="warning-content">
                <p className="warning-text">
                  The Y-axis column "{yAxis}" contains non-numeric values.
                </p>
                <p className="warning-suggestion">
                  For {chartType} charts, Y-axis should contain numbers.
                  Consider switching the columns or selecting a different
                  column.
                </p>
              </div>
            </div>
        ...
        if (!isXNumeric || !isYNumeric) {
          return (
            <div className="data-warning">
              ...
              <div className="warning-content">
                <p className="warning-text">
                  The "{!isXNumeric ? xAxis : yAxis}" column contains
                  non-numeric values.
                </p>
                <p className="warning-suggestion">
                  Scatter/Bubble charts require numeric data for both axes. Try
                  using a Bar or Column chart instead.
                </p>
              </div>
            </div>
          );
        ...
        <div className="chart-container">
          {xAxis && yAxis ? (
            <div className="chart-area">
              {chartType !== "none" && chart3DType === "none" ? (
                  ...
                  !isNumericData(fileData.data, yAxis) ? (
                    <div className="chart-error-state">
                      ...
                      <p>
                        Both selected columns contain text/non-numeric data.
                        Charts require at least one numeric axis to display data
                        properly. Please select a column with numeric values for
                        at least one axis.
                      </p>
                    </div>
                  ) : (
                ...
                <>
                  ...
                  !isNumericData(fileData.data, yAxis) ? (
                    <div className="chart-error-state">
                      ...
                      <p>
                        Both selected columns contain text/non-numeric values.
                        3D charts require numeric data to display properly.
                      </p>
                    </div>
                  ) : (
                    <div className="chart-placeholder">
                      ...
                      <p>
                        X-Axis: {xAxis}, Y-Axis: {yAxis}, Type: {chart3DType}
                      </p>
                    </div>
                  )}
                </>
                ...
                <div className="chart-warning">
                  <FaExclamationTriangle />
                  <p>
                    Please select either a 2D chart type OR a 3D chart type, not
                    both.
                  </p>
                </div>
              ) : (
                <div className="chart-empty-state">
                  ...
                  <p>
                    Choose either a 2D or 3D chart type to visualize your data
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="chart-empty-state">
              ...
              <h3>
                Select axis values and chart type to generate visualization
              </h3>
              <p>
                Choose columns from your data for the X and Y axes and select a
                chart type to visualize your data
              </p>
            </div>
          )}
        </div>
  ...
  const processChartData = useCallback(
    (data, xAxisKey, yAxisKey) => {
      ...
      const backgroundColor = labels.map(
        (_, index) => greenPalette[index % greenPalette.length]
      );
  ...
  const renderChart = () => {
    ...
    */
    ...
    const originalDataLength = Object.keys(
      processChartData(fileData.data, xAxis, yAxis)
    ).length;
    const currentDataLength = processedData.labels.length;
    const samplingInfo =
      originalDataLength > currentDataLength
    ...
    switch (chartType) {
      ...
      case "pie":
        ...
        if ((!isXNumeric && isYNumeric) || (isXNumeric && !isYNumeric)) {
          ...
          pieData.datasets[0].backgroundColor =
            pieData.datasets[0].backgroundColor.map((color) => {
        ...
        if (pieData.labels && pieData.labels.length > 0) {
          ...
          pieData.datasets[0].backgroundColor = items.map(
            (item) => item.backgroundColor
          );
          pieData.datasets[0].borderColor = items.map(
            (item) => item.borderColor
          );
        }

        return <Pie data={pieData} options={specificOptions.pie} />;
      case "doughnut":
        ...
        if ((!dXNumeric && dYNumeric) || (dXNumeric && !dYNumeric)) {
          ...
          doughnutData.datasets[0].backgroundColor =
            doughnutData.datasets[0].backgroundColor.map((color) => {
              // Make colors slightly more muted to indicate non-ideal data
              return color.replace("0.9", "0.7").replace("0.7", "0.5");
            });
        }
        ...
        if (doughnutData.labels && doughnutData.labels.length > 0) {
          ...
          doughnutData.datasets[0].backgroundColor = items.map(
            (item) => item.backgroundColor
          );
          doughnutData.datasets[0].borderColor = items.map(
            (item) => item.borderColor
          );
        }

        return (
          <Doughnut data={doughnutData} options={specificOptions.doughnut} />
        );
      case "scatter":
        return (
          <Scatter
      ...
      case "polar":
        return (
          <PolarArea data={processedData} options={specificOptions.polarArea} />
        );
      case "radar":
        return (
          <Radar
  ...
  const processBubbleData = (data, xAxisKey, yAxisKey) => {
    ...
    if (!isXNumeric || !isYNumeric) {
      console.warn(
        "Bubble chart works best with numeric columns. Some data points may be filtered out."
      );
    }
    ...
    if (validData.length > bubbleLimit) {
      ...
      bubbleData = validData
        .filter((_, index) => index % samplingInterval === 0)
        .slice(0, bubbleLimit);
    }
      ...
      datasets: [
        {
          ...
          hoverBackgroundColor: bubblePoints.map(
            () => "rgba(76, 175, 80, 0.9)"
          ),
          hoverBorderColor: "rgba(46, 125, 50, 1)",
          hoverBorderWidth: 2,
  ...
  useEffect(() => {
    if (fileData && fileData.data && fileData.data.length > 0) {
      ...
      const columnsWithMetadata = headers
        .filter(
          (header) => header && header.trim() !== "" && header !== "__empty"
        )
        .map((header) => {
    ...
    <div className="analyze-data-container">
      ...
      <div className="analyze-content">
        <div className="file-selection-section">
                      ...
                      <div className="dropdown-item-content">
                        ...
                        <div className="file-details">
                          ...
                          <span className="file-meta">
                            {file.metadata?.rowCount || "?"} rows • Uploaded{" "}
          ...
          {files.length === 0 && !loading && !error && (
            <div className="no-files-message">
              <p>
                You haven't uploaded any files yet. Go to the File Upload
                section to upload files for analysis.
              </p>
            </div>
          )}
        </div>
        ...
        {selectedFile && (
          <div className="analysis-options-section">
            ...
            <div className="analysis-options">
              ...
              <div
                className={`option-card ${
                  selectedOption === "visualization" ? "active" : ""
                }`}
              ...
              <div
                className={`option-card ${
                  selectedOption === "filtering" ? "active" : ""
                }`}
              ...
              <div
                className={`option-card ${
                  selectedOption === "advanced" ? "active" : ""
                }`}
        ...
        {selectedFile && selectedOption === "preview" && (
          <div className="data-content-section">{renderDataPreview()}</div>
        )}
      ...
      <div id="cellContentModal" className="cell-content-modal">
        <div className="cell-content-container">
          <div className="cell-content-header">
            ...
            <button
              className="cell-content-close"
              onClick={() =>
                (document.getElementById("cellContentModal").style.display =
                  "none")
              }
            >
              ×
